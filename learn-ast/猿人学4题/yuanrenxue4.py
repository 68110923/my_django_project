...
def encrypt(key,value):
    b = base64.b64encode((key+value).encode())
    str = b.decode().replace('=','')
...
for page in range(1,6):
    ...
    iv = resp['iv']
    info = resp['info']
    bs_obj = BeautifulSoup(info, 'html.parser')
    display_selector = encrypt(key,value)
    page_num_list = []
    for td in bs_obj.find_all('td'):
        ...
        n = 0
        for img in img_list:
            # class =\"img_number 9acafaf61139aa01c239a80532e421f7\" style=\"left:0px\"
            num = num_dic[img['src']]
            if img['class'][1].find(display_selector) == -1:
                n += 1
                z = int(float(img['style'].strip('left:').strip('px'))/11.5)
                num_list[n + z] = num
